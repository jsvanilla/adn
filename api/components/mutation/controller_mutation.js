...
module.exports = function (injectedStore) {
  ...
  async function mutationResult (result = true) {
    if (result = true) {
      return { text: '¡ Se encontró una mutación en la cadena de ADN !', status: 200 }
    } else {
      return { text: 'No se encontró mutación alguna en la cadena de ADN', status: 403 }
    }
  }  

  function upsert ({ id, chain, mutation }) {
    const adn = { id, chain, mutation }
    adn.id = nanoid()
